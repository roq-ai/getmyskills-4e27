...
type ColumnType = ColumnDef<TrainerInterface, unknown>;

interface TrainerListPageProps {
...
export function TrainerListPage(props: TrainerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTrainers({
        ...
        order: params.order,
        searchTermKeys: ['skill_set.contains', 'location.contains', 'experience.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TrainerInterface>>(
    () => `/trainers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TrainerInterface) => {
    if (hasAccess('trainer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/trainers/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'skill_set', header: 'Skill Set', accessorKey: 'skill_set' },
    {
      id: 'availability',
      header: 'Availability',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/trainers/view/${record.id}`} passHref legacyBehavior>
                <Button

...
import { TrainerListPage as ShowCaseList } from 'pages/trainers';
...
function DashboardPage() {
  ...
  const trainerStat = useSWR<PaginatedInterface<TrainerInterface>>(
    () => `/trainers`,
    () => getTrainers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Trainers',
      result: trainerStat?.data?.totalCount,
      isLoading: trainerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`trainers`} passHref legacyBehavior>
                  <Button
